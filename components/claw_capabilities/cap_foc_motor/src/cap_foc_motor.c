                                        ...
                                        char *output, size_t output_size)
{
    ...
    snprintf(output, output_size,
             "FOC motor initialized: PWM=%luHz, GPIO %d/%d/%d, ADC1 CH%d/CH%d, pole_pairs=%u",
    ...
    while (1) {
        ...
        if (s_running && s_hw) {
            ...
            if (s_openloop_mode) {
                ...
                /* Generate SVPWM from open-loop angle with small voltage */
                float valpha = cosf(s_core.theta) * 0.3f;
                float vbeta  = sinf(s_core.theta) * 0.3f;
                float du, dv, dw;
                ...
                /* TODO: Update theta from encoder/sensor here */
                foc_core_openloop_step(&s_core, s_openloop_speed, dt);
...
static const claw_cap_descriptor_t s_foc_motor_descriptors[] = {
    ...
    },
    {
        ...
        .family = "motor",
        .description = "Read current motor state (running, currents, angle, targets).",
        .kind = CLAW_CAP_KIND_CALLABLE,
        .cap_flags = CLAW_CAP_FLAG_CALLABLE_BY_LLM,
        .input_schema_json = "{\"type\":\"object\",\"properties\":{}}",
